...
async function fetchEmployees() {
  try {
    ...
    const res = await axios.get(`${BASE_URL}/employees/?active=true&manager_email=${encodeURIComponent(managerEmail)}`);
  ...
  async function loadRequests() {
    try {
      ...
      // Verificação de segurança para evitar erros
       ...
       const processed = Array.isArray(responseData.processed) ? responseData.processed : [];
  ...
  return `
    <tr>
      <td>${nome}</td>
      <!-- <td>${dataSolicitada}</td> -->
      <td class="text-truncate" style="max-width: 200px;" title="${motivo}">${motivo}</td>
      <td>${criadoEm}</td>
      <td>
